...
module.exports = {
    ...
    _slideToItem: function(index) {
        ...
        var $nextPicsItem = $_.find(".slider-digits__pics__item").eq(
            self._getRealIndex(nextIndex)
        );
        var $nextTextItem = $_.find(".slider-digits__text__item").eq(
            self._getRealIndex(nextIndex)
        );
        ...
        var $currPicsItem = $_.find(".slider-digits__pics__item").eq(
            self._getRealIndex(currIndex)
        );
        var $currTextItem = $_.find(".slider-digits__text__item").eq(
            self._getRealIndex(currIndex)
        );
        ...
        switch (direction) {
            case "left":
                ...
                var position =
                    $nextNumsItem.offset().left - $numsCrop.offset().left;
                $fakeNumsItem
                    .addClass("_fixed")
                    .css({ left: position })
                    .appendTo($numsCrop);
                ...
                var distance =
                    $fakeNumsItem.offset().left - $nextTextValue.offset().left;
            ...
            case "right":
                ...
                var position =
                    $currTextValue.offset().left - $numsCrop.offset().left;
                $fakeNumsItem
                    .addClass("_fixed")
                    .css({ left: position })
                    .appendTo($numsCrop);
                ...
                // get the distance we will animate
                var distance =
                    $numsCrop.offset().left -
                    $currTextValue.offset().left +
    ...
    _bindUI: function() {
        ...
        $(document).on(
            "click",
            ".slider-digits__nums__item",
            { self: self },
            self._handleNumClick
        );
        $(document).on(
            "click",
            "#slider-digits .js-slider-digits-prev",
            { self: self },
            self._handlePrevClick
        );
        $(document).on(
            "click",
            "#slider-digits .js-slider-digits-next",
            { self: self },
            self._handleNextClick
        );
        $(document).on(
            "touchstart",
            ".slider-digits__center",
            { self: self },
            self._handleTouchStart
        );
        $(document).on(
            "touchmove",
            ".slider-digits__center",
